...
# По умолчанию ВЫКЛЮЧЕНО: иначе симулятор сам плодит случайные сценарии, а
# состояние не персистится — при перезапуске/--reload авто-режим снова включался
# бы и засорял ground-truth. Для «живой» демонстрации включается галочкой в WebUI;
# для эксперимента (гл. 4) сценарии запускаются вручную детерминированно.
AUTO_SCENARIOS_ENABLED = False
